...
def analyze_schema_structure(yaml_data: dict) -> dict:
    ...
    for i, node_type in enumerate(node_types):
        ...
        if 'typeLabel' in identifier:
            analysis['node_type_identifiers'][i] = f"typeLabel: {identifier['typeLabel']}"
        ...
        if 'supertypes' in implies and implies['supertypes']:
            ...
            type_name = identifier.get('typeLabel', f"NodeType[{i}]")
    ...
    for i, edge_type in enumerate(edge_types):
        ...
        if 'typeLabel' in identifier:
            analysis['edge_type_identifiers'][i] = f"typeLabel: {identifier['typeLabel']}"
        elif 'typeIdentifyingLabels' in identifier:
        ...
        implies = edge_type.get('implies', {})
        if 'supertypes' in implies and implies['supertypes']:
            analysis['has_supertypes'] = True
            type_name = identifier.get('typeLabel', f"EdgeType[{i}]")
            analysis['types_with_supertypes'].append(f"{type_name} extends {implies['supertypes']}")
