...
- **GQL-schemas / types-graphs directories**: Containers of primary catalog objects that are leaf nodes in the Catalog (GQL-specific terminology; in LEX we prefer "types-graphs directory" to avoid confusion with graphSchema documents)
...
12. WHEN the root directory is also a types-graphs directory THEN the system SHALL allow primary catalog objects to be placed directly at the root level (if catalog support is available)
13. WHEN I define a primary catalog object named "Foo" within a types-graphs directory at my session's default path THEN the system SHALL assign it the fully-qualified name "<default_path>/Foo"
...
4. WHEN I add primary catalog objects THEN the system SHALL place them within types-graphs directories (leaf nodes) with string names unique among their siblings
5. WHEN I reference any types-graphs directory THEN the system SHALL provide a fully-qualified name starting with "/" and consisting of successive directory names separated by "/" and terminating with "/" + types-graphs directory name
6. WHEN I work with directories THEN the system SHALL ensure they can only contain other directories or types-graphs directories
7. WHEN I work with types-graphs directories THEN the system SHALL ensure they can only contain named primary catalog objects (graphs, graph types, tables, procedures, JSON Schema definitions)
8. WHEN I reference a primary catalog object THEN the system SHALL provide a fully-qualified name consisting of the types-graphs directory's fully-qualified pathName + "/" + the primary catalog object's name
9. WHEN I work with a Catalog THEN the system SHALL allow me to store and manage multiple types of named primary catalog objects (graphs, graph types, tables, procedures, JSON Schema definitions) within types-graphs directories
10. WHEN I navigate the Catalog THEN the system SHALL provide filesystem-like operations for traversing directories and accessing types-graphs directories and their contained primary catalog objects
11. WHEN I serialize a Catalog THEN the system SHALL preserve the complete hierarchical structure including all directory paths, types-graphs directories, and named primary catalog objects
...
11. WHEN I work with the Catalog structure THEN the system SHALL recognize it as a tree graph with directories as internal nodes and types-graphs directories as leaf nodes
...
4. WHEN I visualize the Catalog tree THEN the system SHALL show directories, types-graphs directories, and their hierarchical relationships
...
**User Story:** As a developer, I want to use LEX catalog DDL commands to create and manage directory structures and types-graphs directories in the catalog, so that I can organize my schemas hierarchically using DDL syntax that extends GQL's existing capabilities.

**Terminology Note**: In DDL syntax, the keyword is "GQL SCHEMA" (following GQL standard terminology). In LEX conceptual documentation, we use "types-graphs directory" to avoid confusion with graph schemas (graphSchema in YAML). In YAML documents, the top-level type is `graphSchema:`.

**GQL Standard Summary**: GQL provides USE <graph_expression> for specifying working graphs in statements and AT <schema_reference> for schema context in procedures. Schema references support absolute paths like "/production/analytics/schema1" and relative paths like "../schema2", "HOME_SCHEMA", "CURRENT_SCHEMA". However, GQL lacks DDL commands to CREATE/DROP directories or types-graphs directories, and has no SHOW commands for catalog inspection.
...
3. WHEN I use LEX catalog DDL THEN the system SHALL provide CREATE GQL SCHEMA syntax for creating types-graphs directory containers (missing from GQL standard, "GQL SCHEMA" is the DDL keyword)
4. WHEN I use LEX catalog DDL THEN the system SHALL provide DROP GQL SCHEMA syntax for removing types-graphs directory containers (missing from GQL standard, "GQL SCHEMA" is the DDL keyword)
...
13. WHEN I perform directory operations THEN the system SHALL maintain referential integrity for all contained types-graphs directories and their primary catalog objects
...
5. WHEN I reference a types-graphs directory (leaf node) THEN the system SHALL create an IRI by extending the parent directory's IRI with "/" + schema_name
6. WHEN I reference a Primary Catalog Object (PCO) THEN the system SHALL create an IRI by extending the types-graphs directory's IRI with "/" + PCO_name
...
12. WHEN I store LEX schemas in types-graphs directories THEN the system SHALL treat them as extended primary catalog objects
...
**User Story:** As a developer, I want to store data graphs (graph instances) as Primary Catalog Objects in types-graphs directories, with support for both user-defined schemas and schema-free operation, so that I can manage actual graph data alongside schema definitions.

#### Acceptance Criteria

1. WHEN I store a data graph in a types-graphs directory THEN the system SHALL treat it as a Primary Catalog Object (PCO) distinct from graph schemas
