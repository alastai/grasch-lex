...
class ImportPreprocessor:
    ...
    def parse_wrapper(self, data: Any, context: str = "") -> Optional[TypeInterpretationWrapper]:
        ...
        if first_key in self.ONE_LEVEL_WRAPPER_KEYWORDS:
            ...
            if not second_wrapper:
                raise ValueError(
                    f"Incomplete two-level wrapper at {context}. "
                    f"Found {first_key} without concreteness wrapper (concrete/abstract)"
                )
            ...
            if second_key not in self.CONCRETENESS_KEYWORDS:
                raise ValueError(
                    f"Invalid wrapper structure at {context}. "
                    f"Expected concreteness wrapper (concrete/abstract) after {first_key}, "
                    f"found {second_key}"
                )
    ...
    def canonicalize_wrapper(self, data: Any, parent_key: str = None, context: str = "") -> Any:
        ...
        if wrapper:
            # This is a wrapper - canonicalize it
            ...
            if isinstance(wrapped_content, dict):
                # Check if wrapped content contains wrappable properties
                for key in wrapped_content:
                    if key in self.WRAPPABLE_PROPERTIES or key in self.WRAPPABLE_ARRAYS:
                        # Recursively canonicalize
                        wrapped_content[key] = self.canonicalize_wrapper(
                            wrapped_content[key],
                            key,
                            f"{context}.{key}"
                        )
    ...
    def process(self, data: Any, current_path: Path = None, parent_key: str = None) -> Any:
        ...
        if isinstance(data, dict):
            ...
            # Canonicalize list items if they're in a wrappable array
            if parent_key in self.WRAPPABLE_ARRAYS:
                return [self.canonicalize_wrapper(item, parent_key, f"{parent_key}[{i}]") for i, item in enumerate(result)]
...
if __name__ == '__main__':
    ...
    try:
        ...
        sys.exit(1)
                ...
                result.append(self.canonicalize_content(item, item_context))
        
        return result
    
    def canonicalize_endpoint(self, data: Any, context: str = "") -> Any:
        """
        Canonicalize an edge endpoint reference, which may have wrappers.
        
        Args:
            data: The endpoint data
            context: Context string for error messages
            
        Returns:
            Canonicalized endpoint
        """
        if isinstance(data, dict) and self.detect_wrapper(data):
            return self.canonicalize_wrapper(data, context)
        else:
            # Bare endpoint reference - no canonicalization needed for endpoints
            # (they default to exactlyOf: concrete: semantically, but we don't wrap them)
            return data
    ...
    def process(self, data: Any, current_path: Path = None, parent_key: str = None) -> Any:
        ...
        Args:
            data: The data structure to process
            current_path: Path of the current file being processed
            parent_key: The key under which this data appears (for wrapper stripping)
        ...
        if isinstance(data, dict):
            # Check if this is an import-only directive
            if 'import' in data and len(data) == 1:
                return self.resolve_import(data, current_path, parent_key)
            ...
            for item in data:
                if isinstance(item, dict) and 'import' in item and len(item) == 1:
                    ...
                    result.append(self.process(item, current_path, parent_key))
            return result
